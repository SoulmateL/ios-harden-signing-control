...
- 新生产 Key ID 固定为 `skb-integrity-prod-2026-03`，必须对应全新的 seed
  和全新的公钥指纹。
- 恢复副本使用由所有者管理的加密 APFS U 盘，不使用仓库、聊天、截图或普通
  明文文件。
...
- 不在没有所有者现场确认和加密 U 盘的情况下生成生产 seed。
...
2. 本地派生公钥和公钥 SHA-256。
3. 通过标准输入写入 GitHub Secret，不产生 shell history。
4. 将同一 seed 写入已经加密且当前已解锁的 APFS U 盘恢复卷。
5. 同步并卸载恢复卷。
6. 临时目录只允许位于 `mktemp -d` 创建的目录，结束时安全清理。
7. 输出只包含 Key ID、公钥和公钥指纹。

生产初始化前必须由 `SoulmateL` 现场输入 U 盘解锁密码。自动化不得接收、保存或
记录该密码。
...
- U 盘恢复副本不可用：不从日志或制品寻找 seed；生成新 Key ID 并重新轮换。
